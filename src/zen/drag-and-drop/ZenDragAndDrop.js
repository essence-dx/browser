  ...
  window.ZenDragAndDrop = class extends window.TabDragAndDrop {
    ...
    startTabDrag(event, tab, ...args) {
      ...
      if (options.dragImageSource) {
        ...
        draggingTabs = tab.multiselected ? getSelectedTabsSync() : [tab];
      }
    ...
    handle_dragend(event) {
      ...
      // Source window's drag instance (upstream reads it off the tab
      // container; Chromium: same element lookup, no tabbrowser needed).
      let thisFromGlobal =
        ownerGlobal?.document?.getElementById("tabbrowser-tabs")
          ?.tabDragAndDrop ?? this;
