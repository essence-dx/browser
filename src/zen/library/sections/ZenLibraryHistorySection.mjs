...
import { html, repeat } from "../../adapters/lit.mjs";
import {
  MS_PER_DAY,
  PAGE_SIZE,
...
import { importGeckoModule, PLACES_QUERY_URL, PLACES_UTILS_URL } from "../../adapters/gre.mjs";
import { playHapticFeedback } from "../../adapters/prefs.mjs";
// Chromium: history subsystem via the gre loader (real Places modules on
// Gecko, null on Chromium — the section renders empty there until the shell
// wires chrome.history). Haptics via the prefs adapter.

const lazy = {
  PlacesQuery: importGeckoModule(PLACES_QUERY_URL),
  Places: importGeckoModule(PLACES_UTILS_URL),
};
...
Object.defineProperty(lazy, "dateFormat", {
  configurable: true,
  enumerable: true,
  get() {
...
export class ZenLibraryHistorySection extends ZenLibrarySearchSection {
  ...
  connectedCallback() {
    ...
    if (!lazy.PlacesQuery) {
      // Chromium: no history subsystem yet; section stays empty.
      return;
    }
  ...
  #fetchMostVisited(daysOld, limit) {
    ...
    const query = lazy.Places.history.getNewQuery();
    query.beginTime = lazy.Places.toPRTime(
      Date.now() - daysOld * MS_PER_DAY
    );
    const options = lazy.Places.history.getNewQueryOptions();
    options.sortingMode = options.SORT_BY_VISITCOUNT_DESCENDING;
    options.maxResults = limit;
    const root = lazy.Places.history.executeQuery(query, options).root;
    ...
    for (let i = 0; i < root.childCount; i++) {
      ...
      visits.push({
        url: node.uri,
        title: node.title,
        date: lazy.Places.toDate(node.time),
        guid: node.pageGuid,
      });
    }
  ...
  #forgetVisit(visit) {
    lazy.Places?.history.remove(visit.url);
        ...
        <div class="zen-library-row-actions">
          ...
          >
            <img
              ...
              src="../../../browser/themes/shared/zen-icons/nucleo/trash.svg"
              alt=""
            />
          </toolbarbutton>
          <toolbarbutton
          ...
          >
            <img
              class="toolbarbutton-icon"
              src="../../../browser/themes/shared/zen-icons/nucleo/u-turn-to-left.svg"
              alt=""
            />
          </toolbarbutton>
        </div>
