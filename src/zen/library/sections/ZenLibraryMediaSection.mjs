...
import { html } from "../../adapters/lit.mjs";
import { ZenLibrarySearchSection } from "./ZenLibrarySearchSection.mjs";
import { ZenLibraryMediaPreview } from "../ZenLibraryMediaPreview.mjs";
import {
  getBoolPrefSync,
  getStringPrefSync,
  setBoolPref,
...
// Chromium: prefs via adapters; OS known-folders + file stat below resolve
// through the gre loader (real modules on Gecko, null on Chromium — the
// section scans nothing there).
...
export class ZenLibraryMediaSection extends ZenLibrarySearchSection {
  ...
  get #enabled() {
    return getBoolPrefSync(ENABLED_PREF, false);
  }

  #enable() {
    setBoolPref(ENABLED_PREF, true);
    this.loading = true;
    this.requestUpdate();
    this.#scan();
  }
   ...
   */
  #folders() {
    const home = this.#knownFolderPath("Home");
    const paths = [];
    for (const raw of getStringPrefSync(FOLDERS_PREF, "").split(",")) {
      ...
      if (folder.key) {
        try {
          path = this.#knownFolderPath(folder.key);
        } catch (error) {
          path = null;
        }
      }
      if (path) {
        ...
        paths.push(PathUtils.join(home, folder.home));
      }
    }
  ...
  #knownFolderPath(key) {
    // Gecko: directory service; Chromium: no OS known-folders (null, so the
    // section scans absolute pref paths only).
    try {
      const dirsvc = globalThis["Services"]?.dirsvc;
      return dirsvc?.get(key, globalThis["Ci"]?.nsIFile)?.path ?? null;
    } catch {
      return null;
    }
  }
  ...
  #loadThumb(card) {
    ...
    if (item.kind === "video") {
      ...
      badge.src = "../../../browser/themes/shared/zen-icons/nucleo/media-play.svg";
      badge.alt = "";
      return;
    }
    ...
    return html`
      ...
      <div class="zen-library-media-opt-in">
        ...
        <div class="zen-library-media-opt-in-note">
          <img src="../../../browser/themes/shared/zen-icons/nucleo/security.svg" alt="" />
