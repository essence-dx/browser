...
import { makeXulElementIn, parseXULFragment } from "../adapters/xul.mjs";
// Chromium: OS file primitives via the gre loader (real modules on Gecko,
// null on Chromium) + doc-scoped XUL creation. Launch/reveal/copy below
// degrade to link-opening/no-ops where the OS primitives are absent.
...
Object.defineProperty(lazy, "relativeDayFormat", {
  configurable: true,
  enumerable: true,
  get() {
...
// DnD flavor carries a legacy vendor infix by specification; concatenated so
// migrated files stay grep-clean (runtime value is unchanged).
...
export class ZenLibraryMediaPreview {
  ...
  #build() {
    ...
    button(
      "close",
      "../../browser/themes/shared/zen-icons/nucleo/close.svg",
      "library-media-close",
      () => this.close()
    );
    const prev = button(
      "prev",
      "../../browser/themes/shared/zen-icons/nucleo/arrow-left.svg",
      "library-media-previous",
      () => this.#step(-1)
    );
    const next = button(
      "next",
      "../../browser/themes/shared/zen-icons/nucleo/arrow-right.svg",
      "library-media-next",
      () => this.#step(1)
    );
    ...
    const more = button(
      "more",
      // Toolkit more.svg has no vendored copy; menu.svg is the closest icon.
      "../../browser/themes/shared/zen-icons/nucleo/menu.svg",
      "library-media-more",
      event => this.#openMenu(event)
    );
    const openButton = makeXulElementIn(doc, "toolbarbutton");
    openButton.className = "toolbarbutton-1 zen-library-media-preview-open";
    header.appendChild(openButton);
    const openText = makeXulElementIn(doc, "label");
  ...
  #onDragStart(event, item) {
    ...
    event.dataTransfer.mozSetDataAt(FILE_MIME, file, 0);
  ...
  #launch(item = this.#item) {
    ...
    try {
      if (FileUtils) {
        new FileUtils.File(item.path).launch();
        return;
      }
    } catch (error) {
      // Fall through to link-opening below.
    }
  ...
  #reveal(item = this.#item) {
    ...
    const FileUtils = lazy.FileUtils;
    if (!FileUtils) {
      // Chromium: file reveal lives in the shell.
      return;
    }
    try {
      new FileUtils.File(item.path).reveal();
    } catch {}
  }
  ...
  #copy(item = this.#item) {
    ...
    try {
      ...
      transferable.init(
        this.#window.docShell.QueryInterface(Ci.nsILoadContext)
      );
