...
import { html, nothing, repeat, MozLitElement } from "../adapters/lit.mjs";
import { ZenLibraryWidget } from "./ZenLibraryWidget.sys.mjs";
import { ZenLibraryHistorySection } from "./sections/ZenLibraryHistorySection.mjs";
import { ZenLibraryDownloadsSection } from "./sections/ZenLibraryDownloadsSection.mjs";
import { ZenLibraryBoostsSection } from "./sections/ZenLibraryBoostsSection.mjs";
import { ZenLibraryMediaSection } from "./sections/ZenLibraryMediaSection.mjs";
import { ZenLibrarySpacesSection } from "./sections/ZenLibrarySpacesSection.mjs";
import {
  getBoolPrefSync,
  getStringPrefSync,
  setStringPref,
...
// Chromium: lit/directives + sections + prefs + XUL creation via adapters
// (npm lit / chrome.storage / createElement on Chromium).
...
Object.defineProperty(lazy, "appContentWrapper", {
  configurable: true,
  enumerable: true,
  get() {
...
export class ZenLibrary extends MozLitElement {
  ...
  constructor() {
    ...
    const lastTab = getStringPrefSync(LAST_TAB_PREF, "history");
  ...
  set activeTab(value) {
    ...
    setStringPref(LAST_TAB_PREF, value);
  }
  ...
  static toggle(tab = undefined) {
    if (!getBoolPrefSync("zen.library.enabled", false)) {
      return;
    }
  ...
  get #isCompactMode() {
    return (
      ...
      (getBoolPrefSync("zen.view.compact.hide-tabbar", false) ||
        getBoolPrefSync("zen.view.use-single-toolbar", false))
    );
  }
  ...
  #buildFooterButtons() {
    ...
    const buttons = [
      {
        image: "../../browser/themes/shared/zen-icons/nucleo/back.svg",
        l10nId: "library-footer-close-button",
        command: () => ZenLibrary.animateProgress(0),
      },
      {
        image: "../../browser/themes/shared/zen-icons/nucleo/heart-circle-fill.svg",
