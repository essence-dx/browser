...
import { getIntPrefSync, playHapticFeedback } from "../adapters/prefs.mjs";
// Chromium: tab-strip queries + tab moves + prefs + haptics via adapters
// (chrome.tabs / chrome.storage / navigator.vibrate). The strip instance
// below resolves off the tab container element on both engines.
...
export class ZenLibraryDragAndDrop extends window.ZenDragAndDrop {
  ...
  #draggedItem(event) {
    ...
    return isTab(item) || this.#isSplitViewLabel(item) ? item : null;
  }
  ...
  #movingElements(item) {
    ...
    return moving.map(element =>
      isTabGroupLabel(element) ? element.group : element
    );
  }
  ...
  startTabDrag(event, copy) {
    ...
    const strip = getTabContainerSync()?.tabDragAndDrop;
    if (!strip) {
      return;
    }
  ...
  handle_drop(event) {
    ...
    const tabs = moving.flatMap(element =>
      isTab(element) ? [element] : element.tabs
    );
      ...
      if (intoFolder) {
        ...
        if (firstExisting) {
          for (const tab of moving) {
            moveTabElement(tab, () => firstExisting.before(tab));
          }
        }
      } else {
        ...
        for (const tab of moving) {
          moveTabElement(tab, () =>
            dropBefore ? element.before(tab) : element.after(tab)
          );
        }
      }
    }
  ...
  #land(tab, placeBefore) {
    ...
    if (copy) {
      getTabContainerSync()?.tabDragAndDrop?._landDragImageOnElements(
        [copy],
  ...
  #updateDropTarget(event, item) {
    ...
    if (isFolder) {
      const threshold =
        getIntPrefSync("zen.tabs.folder-dragover-threshold-percent", 20) /
        100;
    ...
    if (previous) {
      // Haptic tick; no-op where vibration is unavailable.
      playHapticFeedback();
    }
