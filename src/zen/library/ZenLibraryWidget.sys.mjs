...
import { canDrawThumbnail } from "./ZenLibraryFileTypes.sys.mjs";
import { parseXULFragment, makeXulElement } from "../adapters/xul.mjs";
import { playHapticFeedback } from "../adapters/prefs.mjs";
// Chromium: engine downloads/file/clipboard APIs via the gre loader (real
// modules on Gecko, null on Chromium) + xul/prefs adapters. The stack below
// only fills with live download objects, which Chromium never produces, so
// Chromium renders an empty stack and all actions below no-op there.
...
class ZenLibraryDownloadStack {
  ...
  constructor(button) {
    ...
    this.#data = lazy.DownloadsCommon?.getData(this.#window, true) ?? null;
  ...
  #parse(markup) {
    return parseXULFragment(markup);
  }
  ...
  #onDragStart(event, download) {
    ...
    if (!FileUtils) {
      // Chromium: OS file drag leaves through the shell; nothing to set.
      event.preventDefault();
      return;
    }
  ...
  #contextMenuItems(download) {
    ...
    if (!C) {
      // Chromium: no download subsystem; nothing to offer.
      return [];
    }
    ...
    if (fileExists) {
      items.push({
        ...
        onClick: () => {
          const FileUtils = lazy.FileUtils;
          if (FileUtils) {
            C.showDownloadedFile(new FileUtils.File(download.target.path));
          }
        },
      });
    }
