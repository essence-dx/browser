...
// UrlbarShared/UrlbarResult live in browser content (chrome URLs upstream);
// the same files resolve through the source tree mapping here.
import { UrlbarShared } from "moz-src:///browser/components/urlbar/content/UrlbarShared.mjs";
import { formatValueSync } from "./ZenUBGlobalActions.sys.mjs";
import { ZenSession } from "../sessionstore/ZenSessionManager.sys.mjs";
import { getSelectedTabSync } from "../adapters/tabs.mjs";
import { importGeckoModule } from "../adapters/gre.mjs";
// Chromium: sidebar data + selected tab via adapters/session + adapters/tabs
// (chrome.sessions / chrome.tabs on Chromium); engine urlbar modules below
// resolve on Gecko and stay null on Chromium.
...
const lazy = {
  UrlbarResult:
    importGeckoModule(MOZ_SRC + "browser/components/urlbar/content/UrlbarResult.mjs")
      ?.UrlbarResult ?? null,
  BrowserWindowTracker:
    importGeckoModule(RESOURCE + "modules/BrowserWindowTracker.sys.mjs")
      ?.BrowserWindowTracker ?? null,
  UrlbarPrefs:
    importGeckoModule(MOZ_SRC + "browser/components/urlbar/UrlbarPrefs.sys.mjs")
      ?.UrlbarPrefs ?? null,
  UrlbarProviderOpenTabs:
    importGeckoModule(
      MOZ_SRC + "browser/components/urlbar/UrlbarProviderOpenTabs.sys.mjs"
    )?.UrlbarProviderOpenTabs ?? null,
  ZenSessionData: ZenSession,
};
...
export class ZenUrlbarProviderSidebar extends UrlbarProvider {
  ...
  async startQuery(queryContext, addCallback) {
    const sidebar = lazy.ZenSessionData.getSidebarData();
    if (!sidebar) {
      return;
    }
      ...
      addCallback(
        ...
        new lazy.UrlbarResult({
          ...
          payload: {
            ...
            icon: "../../browser/themes/shared/zen-icons/nucleo/folder.svg",
            path: path.join(" / "),
          },
          suggestedIndex:
            1 +
